...
public:
  ...
  void RemoveEntity(const JsonObject& details) {
    std::string view = details["view"];
    if (view.compare("RemoveFromView") == 0 || view.compare("AddToView") == 0) {
      SendEventToView("UpdateEntityView", details);
    } else if (view.compare("NotAssigned") == 0) {
  ...
  void Notify(const std::string& message) {
    JsonObject eventData;
    eventData["info"] = message;
    SendEventToView("observe", eventData);
  }
...
public:
  ...
  void RemoveEntity(const JsonObject& details) {
    for (int i = 0; i < sessions.size(); i++) {
      static_cast<TransitService*>(sessions[i])->RemoveEntity(details);
    }
  }
  ...
  void Notify(const std::string& message) {
    for (int i = 0; i < sessions.size(); i++) {
      static_cast<TransitService*>(sessions[i])->Notify(message);
    }
  }
