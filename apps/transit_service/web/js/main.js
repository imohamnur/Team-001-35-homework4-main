  ...
  try {
    api.onmessage = function(msg, data)  {
      if ("event" in data) {
        ...
        if (data.event == "UpdateEntityView") {
          updateEntityView(data.details);
        }
...
function displayNotification(data) {
  ...
  if (notifbar.textContent.length > 450) {
...
const onLoad = ( gltf, position, scale, start, duration, details, id ) => {
  ...
  if (details.type in weatherType) {
    group.visible = showWeather;
...
function toggleWeather() {
  ...
  currentWeather.visible = showWeather;
}
...
function updateEntityView(data) {
  console.log(data);
  console.log(models);
  if (data.view == "RemoveFromView") {
    if (data.details.type in weatherType) {
      currentWeather.visible = false;
      delete currentWeather;
    } else {
      entities[data.id].visible = false;
      viewContainer[data.id] = entities[data.id];
      delete entities[data.id];
    }
    $("#entitySelect option[value='" + data.id + "']").remove();
    if (currentView == data.id) {
      currentView = -1;
    }
  } else if (data.view == "AddToView") {
    if (data.details.type in weatherType) {
      entities[data.id].visible = showWeather;
      currentWeather = entities[data.id];
    } else {
      entities[data.id] = viewContainer[data.id];
      delete viewContainer[data.id];
      entities[data.id].visible = true;
    }
    $("#entitySelect").append($('<option value="' + data.id + '">' + data.details.name + '</option>'));
  }
  console.log(models);
